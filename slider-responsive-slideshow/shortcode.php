...
function awl_slider_responsive_shortcode( $post_id ) {
	...
	$slider_id = esc_attr($post_id['id']);
	

	// Retrieve the base64 encoded data
	$encodedData = get_post_meta($slider_id, 'awl_sr_settings_' . $slider_id, true);
	...
	if (is_sr_serialized($decodedData)) {
		...
		update_post_meta($slider_id, 'awl_sr_settings_' . $slider_id, $jsonEncodedData);
		...
		// Assume the data is in JSON format
		 $jsonData = get_post_meta($slider_id, 'awl_sr_settings_' . $slider_id, true);
